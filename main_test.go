...
func TestGetCustomersHandler(t *testing.T) {
	...
	handler := http.HandlerFunc(handlers.GetCustomersHandler)
...
func TestAddCustomerHandler(t *testing.T) {
	...
	rr := httptest.NewRecorder()
	handler := http.HandlerFunc(handlers.PostCustomerHandler)
	handler.ServeHTTP(rr, req)
...
func TestDeleteCustomerHandler(t *testing.T) {
	...
	rr := httptest.NewRecorder()
	handler := http.HandlerFunc(handlers.DeleteCustomerByIdHandler)
	handler.ServeHTTP(rr, req)
...
func TestGetCustomerHandler(t *testing.T) {
	...
	rr := httptest.NewRecorder()
	handler := http.HandlerFunc(handlers.GetCustomerByIdHandler)
	handler.ServeHTTP(rr, req)
